...
std::vector<struct epoll_event> Epoll::event_v;
std::vector<std::shared_ptr<RequestData>> Epoll::fd2reqdata_v;
int Epoll::efd=-1;
TimerManager Epoll::timer_manager;

/*Summary:call epoll_create() and new event_v,fd_v,call by main() 
 * */
int Epoll::epoll_init(){
    efd=epoll_create1(EPOLL_CLOEXEC);
    if(efd==-1)return -1;
    event_v=std::vector<struct epoll_event>(MAXEVENTNUM);
    fd2reqdata_v=std::vector<std::shared_ptr<RequestData>>(MAXFDNUMS);
    return 0;
}
...
int Epoll::epoll_add(int fd,std::shared_ptr<RequestData> request,__uint32_t events){
    ...
    event.data.fd=fd;
    event.events=events;
    fd2reqdata_v[fd]=request;//this fd's requestdata-pointer is request 
    if(epoll_ctl(efd,EPOLL_CTL_ADD,fd,&event)<0){
        perror("epoll_add error");
        fd2reqdata_v[fd].reset();//clear request's obj
        return -1;
    }
    return 0;
}
int Epoll::epoll_mod(int fd,std::shared_ptr<RequestData> request,__uint32_t events){
    ...
    event.data.fd=fd;
    event.events=events;
    fd2reqdata_v[fd]=request;
    if(epoll_ctl(efd,EPOLL_CTL_MOD,fd,&event)<0){
        perror("epoll_del error");
        fd2reqdata_v[fd].reset();//clear request obj 
        return-1;
    }
    return 0;
}
...
/*Summary:gen ju huo yue shi jian chuangjian huo zhe she zhi fd dui ying de requestData duixiang
 * */
std::vector<std::shared_ptr<RequestData>> Epoll::getEventsRequest(int lfd,int events_num){
    ...
    for(int i=0;i<events_num;++i){
        int fd=event_v[i].data.fd;
        if(fd==lfd){//lfd:new conn 
            acceptConnection(lfd);//get new conn 
        }else{//cfd:
            if((event_v[i].events&EPOLLERR)||(event_v[i].events&EPOLLHUP)){
                //error event:
#ifdef DEBUG
                std::cout<<"error event"<<std::endl;
#endif
                if(fd2reqdata_v[fd])fd2reqdata_v[fd]->seperateTimer();
                fd2reqdata_v[fd].reset();
                continue;//next event
            }
            //read OR write event:
            std::shared_ptr<RequestData> curr_req=fd2reqdata_v[fd];
            if(curr_req){
                if(event_v[i].events&EPOLLIN){
                    curr_req->enableRead();
                }
                if(event_v[i].events&EPOLLOUT){
...
#ifdef DEBUG
                std::cout<<"fd2reqdata_v[i] nullptr"<<std::endl;
#endif
            }
        }
    }
...
/*Summary:call epoll_wait() and call getEventsRequest and add every requestdata in thread_pool_task_queue
 * */
void Epoll::my_epoll_wait(int lfd){
    static int time_flag=0;
    int readynum=epoll_wait(efd,event_v.data(),event_v.size(),-1);

    if(readynum<0){perror("epoll wait error");}
    auto req_data=getEventsRequest(lfd,readynum);
    if(!req_data.empty()){
