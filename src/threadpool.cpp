...
int ThreadPool::threadpool_add_task(std::shared_ptr<void> args, std::function<void(std::shared_ptr<void>)> fun){
    ...
    do
    {
        ...
        task_queue[tail].func = fun;
        task_queue[tail].args = args;
        tail = next;
        ++count;

        /* pthread_cond_broadcast */
        if(pthread_cond_signal(&task_queue_not_empty) != 0)//wake one consumer thread
        {
...
void * ThreadPool::threadpool_thread(void *args)
{
    while (true)
    {
        ...
        {
            //thread blocking on condition of TaskQueue is not empty
        ...
        //at least one task in queue,so thread take a task to handle
        ...
        (task.func)(task.args);//call MyHandler:handle request

    }
    --started;//shutdown is true
    pthread_mutex_unlock(&lock);
    printf("a threadpool's thread exit\n");
    pthread_exit(NULL);
    return(NULL);
}
